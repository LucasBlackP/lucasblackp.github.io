...
(function(root, factory) {
  ...
  ZaloPay.call = function() {
    ...
    if (window.ZaloPayJSBridge && window.ZaloPayJSBridge.call) {
      ...
      'writeLog' !== name &&
        writeLog('info', 'ZaloPayJSBridge.call', 'thucmjs ' + name, opt, _callback);
        // writeLog('info', 'ZaloPayJSBridge.call', 'thucmjs test' + name);
  ...
  ZaloPay.scanQRCode = function(opt) {
    if (!isObj(opt)) {
    ...
    // if (isStr(opt.desc) && isNumber(opt.needResult) && isArray(opt.scanType)) {
      ...
      ZaloPay.call('scanQRCode', opt, cb);
      return;
    // }
  };

  /**
   * ZaloPay.launchInternalWeb(url, cb);
   */
  ZaloPay.launchInternalWeb = function(opt) {
    if (!isObj(opt)) {
      writeLog('error', 'ZaloPay.launchInternalWeb', 'Received invalid object');
      return;
    }
    // if (isStr(opt.desc) && isNumber(opt.needResult) && isArray(opt.scanType)) {
      opt = {
        inappurl: opt.inappurl
      };
      ZaloPay.call('launchInternalWeb', opt, cb);
      return;
    // }
  };

  /**
   * ZaloPay.openCamera(cb);
   */
  ZaloPay.openCamera = function(cb) {
    ZaloPay.call('openCamera', {}, cb);
  };

  /**
   * ZaloPay.chooseImage(cb);
   */
  ZaloPay.chooseImage = function(cb) {
    ZaloPay.call('chooseImage', {}, cb);
  };
  ...
  ZaloPay.hideNavigationButtons = function(opt) {
    if (!isObj(opt)) {
      writeLog('error', 'ZaloPay.hideNavigationButtons', 'Received invalid object');
      return;
    }

    opt = {
      listButtons: opt.listButtons
    };
